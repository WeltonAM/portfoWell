...
export default function JanelaChat() {
    ...
    return (
        <div className="flex flex-col bg-zinc-300 rounded-2xl text-black overflow-hidden">
            ...
            {mensagens.length === 0 ? (
                <div className="flex flex-col justify-center items-center min-h-[400px] sm:min-h-[500px] gap-4 p-4">
                    <IconMessages size={230} stroke={0.2} className="text-black/30" />
                    <span>Vamos conversar?</span>
                </div>
            ) : (
                <div className="flex flex-col p-2 gap-2 min-h-[400px] sm:min-h-[500px] overflow-y-scroll">
            ...
            <div className="flex items-center gap-2 p-1 rounded-full h-10 bg-white m-4">
                <input
                    ...
                    value={texto}
                    className="
                        flex-1 bg-transparent h-8
                        outline-none pl-3
                    "
                    onChange={(e) => setTexto(e.target.value)}
                    onKeyDown={(e: any) => { if (e.key === "Enter") enviarMensagem() }}
                />

                <button className="bg-green-600 flex justify-center items-center min-h-8 min-w-8 rounded-full cursor-pointer hover:scale-105" onClick={() => enviarMensagem()}>
